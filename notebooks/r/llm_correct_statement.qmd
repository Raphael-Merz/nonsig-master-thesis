...
```{r}
data_path <- "../../data/model_performance/test_predictions.csv" # path to where the csv with classifier-labeled statements is

corrected_path <- "../../data/llm_correction_check/corrected_clean.csv"# path to where the final csv with the corrected statements should be stored

labeled_data <- read.csv(data_path)

pred_1_ids <- which(labeled_data$BERT_label == 1)
```
...
```{r}
# not best Practice, but for now set llm_max_calls() to number of statements I will send
llm_max_calls(length(pred_1_ids))

# send each incorrect statement from the labeled dataset with above prompt to the llm
llm_response <- llm(text = labeled_data$statement[pred_1_ids], query = prompt)
...
labeled_data$corrected[pred_1_ids] <- llm_response$answer
...
comparison <- labeled_data[!is.na(labeled_data$corrected), c("statement", "corrected")]
