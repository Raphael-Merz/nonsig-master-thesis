...
Lastly I reviewed 100 statements that were sent to an LLM to be corrected, to see if the LLM-revised statements were actually correct. Of these 100 statements, 80 had previously been labeled incorrect and 20 correct by me, to check how the LLM deals with false positives from the automated classification. I used X different prompts, which had between X and Y words. In short, the model was first explained that it would get a statement with at least one misinterpretations of a nonsignificant finding as the absence of an effect and then instructed to change only parts of the statements that relate to this misinterpretations, but keep the rest of the statement unchanged. I later added XXX. The full promps are available on GitHub (see Section 2.3). Papercheck uses the Groq API (available at <https://groq.com/>) to enable users to communicate with different LLMs. I, in turn, used Papercheck's default model: 'llama-3.3-70b-versatile' (as of 07/24/2025).
...
Since SciBERT achieved the highest performance in the classification task, @tbl-false-classification presents examples of its misclassifications to illustrate the types of sentences that posed difficulties for the model. Potential reasons for these errors will be examined in the discussion.
...
-CITE AND EXPLAIN @bucher_martini24!!! MAYBE USE LLMS FOR EVERYTHING IN THE FUTURE?

-PARAGRAPH ON THE CLASSIFIERS' PROBLEMS WITH SOME OF THE STATEMENTS
...
**COMMENT FROM DANIEL**: Add "There is a surprising lack of conceptual work on how p values should be interpreted correctly, given the many papers focusing on their misinterpretations. For example, should is ‘no significant difference’ a correct interpretation, when some researchers have suggested to abandon the word ‘significant’?"
