...
As shown in @fig-2, the training loss consistently decreased over time for all three models, which is expected given that the models were optimized to fit the training data. The validation loss, however, followed a slightly different trajectory. Visually, standard BERT exhibits a somewhat distinct pattern compared to SciBERT and PubMedBERT, likely reflecting its different pretraining on general English rather than scientific texts. This difference in pretraining may also explain the similar behavior of SciBERT and PubMedBERT in both their performance and the shape of their loss curves over the course of training.
...
![Training and Validation Loss Curve](../data/model_performance/loss_curve/loss_combined.png){#fig-2 width="6in" fig-alt="Curves of the training and validation loss of the three trained BERT models. The best models for regular BERT, SciBERT and PubMedBERT were chosen after 4, 5, and 5 respectively, based on the minumum validation loss." apa-note="Curves of the training and validation loss of the three trained BERT models. The best models for regular BERT, SciBERT and PubMedBERT were chosen after epoch 4, 5, and 5 respectively, based on the minumum validation loss."}

The different performance metrics are summarized in @tbl-model-performance. As reflected in the macro F1 score, both SciBERT and PubMedBERT achieved strong overall performance in classifying correct and incorrect statements with a macro F1 score of `r macro_F1_SciBERT` and `r macro_F1_PubMedBERT`, respectively. Standard BERT lagged slightly behind, with a macro F1 score of `r macro_F1_BERT`.

Since the performance of SciBERT and PubMedBERT was so similar, a closer look at the confusion matrices in @fig-3 reveals subtle differences: PubMedBERT was slightly better at correctly classifying correct statements (with one more true negative), while SciBERT performed slightly better at identifying incorrect statements (with two fewer false positives). Taken together, SciBERT demonstrated the best overall performance. X NEEDS TO BE UPDATES X
...
# Create the data
tibble(
  'Model Prediction'  = c("False Negative", "False Negative" ,"False Positive", "False Positive"),
  'Statement'         = c(test_predictions_SciBERT$statement[74], test_predictions_SciBERT$statement[141],
                          test_predictions_SciBERT$statement[62], test_predictions_SciBERT$statement[91])) %>%
  flextable() %>%
